...
fn main() {
    ...
    let mut running_apps = get_all_running_apps();
    let active_windows = get_active_windows();
    // let active_windows_map = get_active_windows_map();
    // let mut all_running_apps = get_all_running_apps();
    // for (i, app) in all_running_apps.iter() {

    // all_running_apps[i].is_hidden = active_windows_map.contains_key(&app.pid);
    // }
    ...
    running_apps.drain();
    running_apps = get_all_running_apps();
    // for window in active_windows.iter() {
    // if let Some(app) = all_running_apps.get(&window.pid) {
    // running_apps.insert(window.pid, app);
    // }
    ...
    match app_state.mode {
        ...
        ToggleMode::Show => {
            ...
            for (pid, app) in running_apps.iter() {
                if app.is_hidden {
                    // Ignore if it was already hidden
            ...
            /*for window_pid in app_state.windows.iter() {
...
fn get_all_running_apps() -> HashMap<usize, RunningApp> {
    ...
    for i in 0..n_apps {
        ...
        let is_hidden = !active_windows_map.contains_key(&pid);
...
fn get_active_windows_map() -> IndexMap<usize, ActiveWindow> {
    const OPTIONS: CGWindowListOption =
        kCGWindowListOptionOnScreenOnly | kCGWindowListExcludeDesktopElements;
    let window_list = unsafe { CGWindowListCopyWindowInfo(OPTIONS, kCGNullWindowID) };
    let n_windows = unsafe { CFArrayGetCount(window_list) };
    ...
    for i in (0..n_windows).rev() {
        ...
        // Stop coming at me.  Came here to have a good time.  Ignore it!
        if let Some(pid) = pid {
            windows.insert(pid, ActiveWindow { name, pid });
        }
    }
